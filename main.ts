namespace SpriteKind {
    export const poppy = SpriteKind.create()
    export const tank = SpriteKind.create()
}
controller.B.onEvent(ControllerButtonEvent.Pressed, function () {
    bomb = sprites.createProjectileFromSprite(img`
        . . . . . . . . . . . . . . . . 
        b b . . . b b b . . . . . . . . 
        . b b . b b . . . . . . . . . . 
        . b b b b b . . . . . . . . . . 
        . . . b b b b . . . . . . . . . 
        . . b d d d d b . . . . . . . . 
        . b b d d d d b b . . . . . . . 
        . b b d d d d b b . . . . . . . 
        . b d d d b b b b b . . . . . . 
        . b b b b b d d d b . . . . . . 
        . . . b d d d d d d . . . . . . 
        . . . b d d d c c c b . . . . . 
        . . . . b d d c c c b . . . . . 
        . . . . b b b b b b b . . . . . 
        . . . . . b b b b b . . . . . . 
        . . . . . . b b b . . . . . . . 
        `, spitfire, 50, 50)
})
controller.A.onEvent(ControllerButtonEvent.Pressed, function () {
    bullet_from_spitfire = sprites.createProjectileFromSprite(img`
        . . . . . . . . . . . . . . . . 
        . . . . . . . . . . . . . . . . 
        . . . . . . . . . . . . . . . . 
        . . . . . . . . . . . . . . . . 
        . . . . . . . . . . . . . . . . 
        . . . . . . . . . . . . . . . . 
        . . . . 4 . . . . . . . . . . . 
        . . . . 4 4 4 5 5 . . . . . . . 
        . . . . 4 4 4 5 5 5 . . . . . . 
        . . . . 4 4 4 5 5 . . . . . . . 
        . . . . 4 . . . . . . . . . . . 
        . . . . . . . . . . . . . . . . 
        . . . . . . . . . . . . . . . . 
        . . . . . . . . . . . . . . . . 
        . . . . . . . . . . . . . . . . 
        . . . . . . . . . . . . . . . . 
        `, spitfire, 60, -10)
})
sprites.onOverlap(SpriteKind.Food, SpriteKind.Player, function (sprite, otherSprite) {
    if (true) {
        spitfire.destroy(effects.spray, 500)
        game.over(false)
    }
})
sprites.onOverlap(SpriteKind.Projectile, SpriteKind.tank, function (sprite, otherSprite) {
    if (is_tank) {
        tank.destroy(effects.spray, 500)
        is_tank = false
        info.changeScoreBy(20)
    }
})
sprites.onOverlap(SpriteKind.Projectile, SpriteKind.Enemy, function (sprite, otherSprite) {
    if (is_plane) {
        BF109.destroy(effects.spray, 500)
        is_plane = false
        info.changeScoreBy(10)
    }
})
sprites.onOverlap(SpriteKind.Player, SpriteKind.Enemy, function (sprite, otherSprite) {
    if (true) {
        spitfire.destroy(effects.spray, 500)
        game.over(false)
    }
})
let projectile2: Sprite = null
let tank_shell: Sprite = null
let BF109: Sprite = null
let tank: Sprite = null
let bullet_from_spitfire: Sprite = null
let bomb: Sprite = null
let is_tank = false
let is_plane = false
let spitfire: Sprite = null
scene.setBackgroundImage(img`
    
    `)
spitfire = sprites.create(assets.image`spitfire`, SpriteKind.Player)
controller.moveSprite(spitfire)
spitfire.setStayInScreen(true)
is_plane = false
is_tank = false
spitfire.setPosition(14, 51)
game.onUpdateInterval(2000, function () {
    if (is_tank) {
        tank_shell = sprites.createProjectileFromSprite(img`
            . . . . . . . . . . . . . . . . 
            . . . . . . . . . . . . . . . . 
            . . . . 2 2 . . . . . . . . . . 
            . . . . 2 2 2 . . . . . . . . . 
            . . . . . 2 2 2 . . . . . . . . 
            . . . . . . 2 2 2 2 2 . . . . . 
            . . . . . . . 2 2 2 2 . . . . . 
            . . . . . . . 2 2 2 2 . . . . . 
            . . . . . . . 2 2 2 2 . . . . . 
            . . . . . . . . . . . . . . . . 
            . . . . . . . . . . . . . . . . 
            . . . . . . . . . . . . . . . . 
            . . . . . . . . . . . . . . . . 
            . . . . . . . . . . . . . . . . 
            . . . . . . . . . . . . . . . . 
            . . . . . . . . . . . . . . . . 
            `, tank, -40, -29)
        tank_shell.setKind(SpriteKind.Food)
    } else if (is_tank == false) {
        tank = sprites.create(img`
            . . . . . . . . . . . . . . . . 
            . b . . . . . . . . . . . . . . 
            . . b . . . . . . . . . . . . . 
            . . . b . . . . . . . . . . . . 
            . . . . b . . . . . . . . . . . 
            . . . . b b b b b b b b . . . . 
            . . . . . b b b b b b b b . . . 
            . . . . . b b b b b b b b b b b 
            . . . . b b b 2 b b b 2 b b b b 
            . . . 8 8 b b 2 2 b 2 2 b b b b 
            . . b b b b b b 2 b 2 b b b b b 
            . . b b b b b b 2 2 2 b b b b b 
            . b f f f f f f f f f f f f f b 
            b f f b b f b b f b b f b b f f 
            . f f b b f b b f b b f b b f f 
            . . f f f f f f f f f f f f f . 
            `, SpriteKind.tank)
        tank.setPosition(randint(40, 133), 96)
        is_tank = true
    }
})
game.onUpdateInterval(500, function () {
    if (is_plane && Math.percentChance(65)) {
        projectile2 = sprites.createProjectileFromSprite(img`
            . . . . . . . . . . . . . . . . 
            . . . . . . . . . . . . . . . . 
            . . . . . . . . . . . . . . . . 
            . . . . . . . . . . . . . . . . 
            . . . . . . . . . . . . . . . . 
            . . . . . . . . . 5 . . . . . . 
            . . . . . 4 4 4 5 5 . . . . . . 
            . . . . 4 4 4 4 5 5 . . . . . . 
            . . . . . 4 4 4 5 5 . . . . . . 
            . . . . . . . . . 5 . . . . . . 
            . . . . . . . . . . . . . . . . 
            . . . . . . . . . . . . . . . . 
            . . . . . . . . . . . . . . . . 
            . . . . . . . . . . . . . . . . 
            . . . . . . . . . . . . . . . . 
            . . . . . . . . . . . . . . . . 
            `, BF109, -30, 0)
        projectile2.setKind(SpriteKind.Food)
    } else if (is_plane == false) {
        BF109 = sprites.create(img`
            ................................................................
            ................................................................
            ................................................................
            ................................................................
            ................................................................
            ................................................................
            ................................................................
            ................................................................
            ................................................................
            ................................................................
            ................................................................
            ................................................................
            ................................................................
            ................................................................
            ................................................................
            ................................................................
            ................................................................
            ................................................................
            ................................................................
            ................................................................
            ................................................................
            ................................................................
            .....................................................ffff.......
            ....................................................ffbbff......
            ....2..............................................fbbbbbf......
            ....2.............................................ffbbbbbf......
            ....f.............99f9f9ffffffff...............ffffbbbbbbff.....
            ....f...........9999f9f98dddffff..............ffbbbbbbbbbbf.....
            ....f......ffffff888f8ff8dddffffffff.........ffbbbbbbbbbbbff....
            ....f.ffffff7dddddddddddddddffffffffffffffbffbbbbbbbbbbbbbbf....
            ....fff77777dddddddddddddddffffffffffffffbbbbbbbbbbbbbbbbbbf....
            ....f2277777ddddddddddddddfffffffff1f1fffbbbbbbbbbbbbbbbbbff....
            ...f22277777dddddddddddffffffffffff1f1fffbbbbbbbbbbbbbbbbbf.....
            ....f2277777dddddddddffbbbfffffff111f111fbbbbbbbbbbbbbffff......
            ....fff7777ddddddddffbbbbbbbfffffffffffffbbbbbbbbbbffff.........
            ....f.ff77ddddddddffbbbbbbbbbfffff11f111fbbbbbffffff............
            ....f.ffffffdddddbbbbbbbbbbbbffffff1f1fffbfffff.................
            ....f......fddddfbbbbbbbbbbbbbbbfff1f1fffff.....................
            ....2......ffffffffffffffbbbbbbffffffff.........................
            ....2...................ffffffffffffff..........................
            ..............................ff................................
            ................................................................
            ................................................................
            ................................................................
            ................................................................
            ................................................................
            ................................................................
            ................................................................
            ................................................................
            ................................................................
            ................................................................
            ................................................................
            ................................................................
            ................................................................
            ................................................................
            ................................................................
            ................................................................
            ................................................................
            ................................................................
            ................................................................
            ................................................................
            ................................................................
            ................................................................
            ................................................................
            `, SpriteKind.Enemy)
        BF109.changeScale(-0.1, ScaleAnchor.TopRight)
        BF109.follow(spitfire, 0)
        BF109.setPosition(110, randint(10, 60))
        is_plane = true
    }
})
